...
app.post('/api/users/assessor', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Erro ao cadastrar assessor' });
  }
});

// Vincular aluno a assessor
...
app.delete('/api/users/assessor/:assessorId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Erro ao remover assessor' });
  }
});
...
app.get('/api/users/assessores/:admId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar assessores' });
  }
});
...
app.put('/api/tasks/:taskId/status', async (req, res) => {
  ...
  try {
    console.log(`🔄 Atualizando tarefa: ${taskId} para ${status}`);
    ...
    if (result.records.length === 0) {
      console.log(`❌ Tarefa ${taskIdNum} não encontrada`);
    ...
    console.log(`📝 Tarefa: ${taskName}, Status antigo: ${oldStatus}`);
    ...
    await runQuery(
      ...
      RETURN t
      `,
      { id: taskIdNum, status, justification: justification || '' }
    );
    ...
    if (xpGanho > 0) {
      ...
      console.log(`✅ +${xpGanho} XP`);
    }

    console.log(`✅ Tarefa ${taskIdNum} atualizada para ${status}`);
...
app.delete('/api/tasks/:taskId', async (req, res) => {
  ...
  try {
    const taskIdNum = parseInt(String(taskId).split('.')[0]);
...
app.put('/api/questions/:questionId/answer', async (req, res) => {
  ...
  try {
    await runQuery(
      ...
      RETURN q
      `,
      { questionId: parseInt(String(questionId).split('.')[0]), answer }
    );
...
app.post('/api/schedules', async (req, res) => {
  ...
  try {
            ...
            try {
              ...
              await runQuery(
                ...
                CREATE (l:Log {
                  ...
                  oldStatus: '-',
                  newStatus: 'Pendente',
                  justification: 'Tarefa criada automaticamente pelo horário',
                  alunoName: $alunoName,
                  taskName: $taskName
                })
                CREATE (aluno)-[:TEM_LOG]->(l)
    ...
    res.json({ 
      ...
      message: `Horário criado com ${tarefasCriadas.length} tarefas geradas!`
    });
  } catch (error) {
...
async function initDatabase() {
  ...
  try {
    if (!driver) {
      console.error('❌ Driver não disponível');
      return;
    }
